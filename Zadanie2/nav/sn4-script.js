...
function initBigGallery() {
    let tilts = objectToArray(document.querySelectorAll('.sn5_container_gallery_img-positioner'));
...
function createBigGalleryNav(bg, idx, IMG) {
    ...
    nav.classList.add('sn5_bigBG_nav');
    bg.appendChild(nav);
    let a = document.createElement('div');
    a.classList.add('sn5_bigBG_nav_positioner');
    nav.appendChild(a);
    ...
    btns.forEach(function (btn, i) {
        ...
        btn.classList.add('sn5_bigBG_nav_positioned-btn');
        a.appendChild(btn);
        btn.innerHTML = icons[i];
        let images = objectToArray(document.querySelectorAll('.sn5_container_gallery_img-positioner'));
        let idLetters = getBackgroundImageNames(images);
        if (i === 0) {
            letThisWork(btn, IMG, idLetters, (-1));
        } else if (i === 1) {
            btn.addEventListener('click', function () {
                document.querySelector('.sn5_bigBG').classList.add('sn5_fade-out');
                setTimeout(function(){
                    document.body.removeChild(document.querySelector('.sn5_bigBG'));
                },300);
            });
        } else if (i === 2) {
            letThisWork(btn, IMG, idLetters, 1);
...
function letThisWork(btn, img, ids, dir) {
    btn.addEventListener('click', function () {
        ...
        img.classList.remove('sn5_fade-out-in');
        setTimeout(function () {img.classList.add('sn5_fade-out-in')},0);
        setTimeout(function () {
